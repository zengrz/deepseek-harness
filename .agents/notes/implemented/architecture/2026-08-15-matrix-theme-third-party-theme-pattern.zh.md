...
主题服务接受第三方主题注册，但在 Matrix 主题之前没有任何插件使用过该扩展点：第三方主题的 token 表面、选择 UI、氛围呈现与偏好生命周期都没有先例，而内建的外观行刻意只列出 `light`/`dark`/`system`。

## Decision

`@deepseek-ai/dsh-client-ui-matrix-theme` 提供《黑客帝国》风格主题，同时充当第三方主题贡献的模板：
...
- 主题以可安装的 profile 插件交付——`@deepseek-ai/dsh-matrix-theme`，用 `dsh plugin --profile <name> add @deepseek-ai/dsh-matrix-theme` 安装——绝不作为随箱的 web-app 花名册行；[standalone-bundle note](../feature/2026-08-16-matrix-theme-standalone-bundle.md) 拥有交付方式，本 note 拥有该模式。
