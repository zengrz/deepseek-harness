...
The theme service accepts third-party theme registrations, but before the Matrix theme no shipped plugin exercised the extension point: nothing defined what a third-party theme's token surface, selection UI, ambient presentation, and preference lifetime look like, and the built-in Appearance row deliberately lists only `light`/`dark`/`system`.

## Decision

`@deepseek-ai/dsh-client-ui-matrix-theme` ships the Matrix movie aesthetic theme and doubles as the template for third-party theme contributions:
